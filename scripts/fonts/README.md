...
Regenerate with fonttools if the heading words or ramp ever change:

```
uv run fonttools subset JetBrainsMono-Regular.ttf --text=' .`:-=+*cs#%@' \
  --flavor=woff2 --no-hinting --desubroutinize --layout-features= \
  --no-notdef-outline --drop-tables+=DSIG --name-IDs= --name-legacy \
